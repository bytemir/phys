...
static HudProperties properties = { 0.0f, 0.0f, false };
...
void RenderHud(void) {
    ...
    if (properties.ballMenuEnabled) {
        ...
        GuiSliderBar((Rectangle){ 0.75f*SCREEN_WIDTH + 2.85f*FONT_SIZE, 0.25f*SCREEN_HEIGHT+FONT_SIZE, 120, FONT_SIZE }, "Radius (m)", TextFormat("%.2f", properties.objectDimension), &properties.objectDimension, 0, 100);
